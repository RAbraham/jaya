...
def dmp_mapper(line):
    return [('dmp_firehose', [{'name': 'Rajiv', 'age': '65'}, {'name': 'Harry', 'age': '72'}]
             )]
...
class DmpTestCase(unittest.TestCase):
    ...
    #         >> in_process_s3 \
    #         >> mapper \
    #         >> {dmp_firehose_name: dmp_firehose >> dmp_table
    #             }
    #
    ...
    def test_dmp_firehose(self):
        ...
        '''
        db-user = thescore
        db-server = tsa-test-redshift-cluster.coxi26yhkjnd.us-east-1.redshift.amazonaws.com
        db-passwd = VKX2U8O06Gf41m
        db-name = dev
        db-port = 5439
        '''
